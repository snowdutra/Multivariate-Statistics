...
```{r}
ggplot(dados, aes(y = VOLUME, x= IPCA)) + 
...
```{r}
ggplot(dados, aes(y = VOLUME, x= JUROS)) + 
...
```{r}
ggplot(dados, aes(y = VOLUME, x= PIB)) + 
...
```{r}
ggplot(dados, aes(y = VOLUME, x= POUP)) + 
...
```{r}
ggplot(dados, aes(y = VOLUME, x= EMBI)) + 
...
modelo1=lm(VOLUME~IPCA+JUROS+PIB+POUP+EMBI,
data=dados)
...
modelo2=lm(VOLUME~IPCA+JUROS+PIB+EMBI,
data=dados)
...
```{r}
resumo = summary(modelo2) # o número do modelo vai ser o mesmo que utilizou para o teste de hipóteses anterior
tabela4 = round(resumo$coefficients,3) # guarda os coeficientes apenas com três casas decimais
tabela5 = round(confint(modelo2, level = 0.90),3) # guarda o LI e LS com 90% de confiança para os coeficientes apenas com três casas decimais
...
stepAIC(modelo2, direction="backward") # o número do modelo vai ser o mesmo que utilizou para o teste de hipóteses anterior
...
$$
```{r Final, echo=FALSE}
modelo3=lm(VOLUME~JUROS+EMBI,data=dados) # o número do modelo aqui depende de qtos modelos anteriores você fez! Coloque apenas as quantis que sobraram após o método de seleção de variáveis

resumo=summary(modelo3) # o número do modelo aqui deve ser ajustado se  necessário

tabela6=round(resumo$coefficients,3) 
tabela7=round(confint(modelo3,level=0.90),3) 
# PARA MOSTRAR AS TABELAS COM OS RESULTADOS FORMATADOS:
...
cat("O valor do R-quadrado AJUSTADO é: ", 
round(resumo$adj.r.squared*100,2),"%")
```
