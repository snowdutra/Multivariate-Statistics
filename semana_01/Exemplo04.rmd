...
kable(tab_correl, caption = "Matriz de correlações entre variáveis quantitativas") # nolint
```
...
kable(multicolinearidade2, caption = paste("VIF após remover", variavel_removida)) # nolint
```
...
if (valor_p_intercepto > 0.05) {
  ...
  kable(tabela8, caption = "Coeficientes e teste de hipóteses do modelo final sem intercepto") # nolint
  kable(tabela9, caption = "Intervalo de confiança dos coeficientes do modelo final sem intercepto") # nolint
} else {
...
' # nolint
Intercepto (67,936):  # nolint
Representa o tempo estimado de volta (em segundos) para um carro com potência zero, sem aerofólio e com pneu macio. # nolint
Observação: No contexto, potência zero não é possível, então o intercepto não tem interpretação prática direta.  # nolint
'

' # nolint
potencia (-0,026): # nolint 
Para cada aumento de 1 cv na potência do motor, o tempo de volta diminui, em média, 0,026 segundos, mantendo as demais variáveis constantes. # nolint
'  # nolint

' # nolint
aerofoliosim (-2,844):  # nolint
Carros com aerofólio completam a volta, em média, 2,844 segundos mais rápido do que carros sem aerofólio, mantendo as demais variáveis constantes.  # nolint
' # nolint

'  # nolint
pneurigido (3,593):
Carros com pneu rígido levam, em média, 3,593 segundos a mais para completar a volta do que carros com pneu macio, mantendo as demais variáveis constantes.  # nolint
'
```
...
kable(tabela8, caption = "Estimativas dos coeficientes do Modelo 5 (centralizado)") # nolint
kable(tabela9, caption = "Intervalo de confiança dos coeficientes do Modelo 5 (centralizado)") # nolint
